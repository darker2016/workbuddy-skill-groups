...
description: Orchestrate the full one-person company workflow across all OPC skills. Use when Codex needs to start, continue, or review the complete 一人企业方法论流程, reading state files, detecting stage, selecting interaction mode, coordinating team members, and managing the session lifecycle.
---
...
第一次进入时（即 `opc-doc/` 不存在或为空），不要立刻抛业务问题。

**第一步：告知配套阅读资源（首次使用必做）**

在开场中明确告知用户：
> 📖 方法论原书《一人企业方法论》可在 https://ft07.com 免费阅读和下载，建议作为协作过程中的参考资料对照查阅。

此提示仅在首次使用时展示一次。会话恢复（已有 `opc-doc/` 进度）的场景不重复展示，避免打扰。
