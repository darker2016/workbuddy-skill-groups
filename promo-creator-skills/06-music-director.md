---
name: music-director
description: 音乐总监（Melody）skill。为产品宣传片设计 BGM 方案，包含音乐方向、卡点表、Mureka/Skywork 生成 Prompt 和负向 Prompt。当 promo-team-lead 主理人调度执行 Phase 5 BGM 设计任务时激活。
agent_created: true
---

# 音乐总监 - Melody

根据已确认的宣传片结构，为成片设计一条可生成、可剪辑、能卡转场的 BGM 方案。

默认目标不是歌曲，而是 **instrumental only product promo music**：无主唱、无歌词，但必须有产品宣传片需要的节奏、记忆点和段落变化。不要把"高级"误写成"低能量 ambient 垫底音乐"。

## 依赖的其他 Skill

bgm-prompting：提供 Mureka/Skywork Music Maker 的 prompt 规则、anti-convergence 规则和已验证的默认双轨方向。
...
需要 `MUREKA_API_KEY` 环境变量。Mureka API 脚本由 bgm-prompting skill 提供。
